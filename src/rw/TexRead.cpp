...
#ifdef WEBOS_TOUCHPAD
	if (log) { fprintf(log, "RwTextureGtaStreamRead: Found TEXTURENATIVE chunk, size=%u\n", size); fflush(log); }
#endif
...
#ifdef WEBOS_TOUCHPAD
	if (log) {
		fprintf(log, "RwTextureGtaStreamRead: About to call READNATIVE, size=%u, stream=%p\n", size, stream);
		#ifdef LIBRW
		fprintf(log, "RwTextureGtaStreamRead: Using LIBRW rwNativeTextureHackRead\n");
		#else
		fprintf(log, "RwTextureGtaStreamRead: Using standard RenderWare native read\n");
		#endif
		fflush(log);
	}
#endif
...
#ifdef WEBOS_TOUCHPAD
		if (log) { fprintf(log, "RwTextureGtaStreamRead: READNATIVE returned false!\n"); fflush(log); fclose(log); }
#endif
		return nil;
	}
#ifdef WEBOS_TOUCHPAD
	if (log) { fprintf(log, "RwTextureGtaStreamRead: READNATIVE succeeded, tex=%p\n", tex); fflush(log); fclose(log); }
#endif
...
RwTexDictionaryGtaStreamRead(RwStream *stream)
{
	...
	if(!RwStreamFindChunk(stream, rwID_STRUCT, &size, &version))
		return nil;
#ifdef WEBOS_TOUCHPAD
	if (log) { fprintf(log, "RwTexDictionaryGtaStreamRead: Found STRUCT chunk\n"); fflush(log); }
#endif
	if(RwStreamRead(stream, &numTextures, size) != size)
		return nil;

#ifdef WEBOS_TOUCHPAD
	if (log) { fprintf(log, "RwTexDictionaryGtaStreamRead: numTextures=%d\n", numTextures); fflush(log); }
#endif
	...
	if(texDict == nil)
		return nil;
#ifdef WEBOS_TOUCHPAD
	if (log) { fprintf(log, "RwTexDictionaryGtaStreamRead: TexDictionary created\n"); fflush(log); }
#endif

	while(numTextures--){
#ifdef WEBOS_TOUCHPAD
		if (log) { fprintf(log, "RwTexDictionaryGtaStreamRead: Reading texture %d...\n", numTextures); fflush(log); }
#endif
		tex = RwTextureGtaStreamRead(stream);
		if(tex == nil){
#ifdef WEBOS_TOUCHPAD
			if (log) { fprintf(log, "RwTexDictionaryGtaStreamRead: RwTextureGtaStreamRead returned NULL!\n"); fflush(log); fclose(log); }
#endif
...
#ifdef WEBOS_TOUCHPAD
		if (log) { fprintf(log, "RwTexDictionaryGtaStreamRead: Texture read OK, adding to dict...\n"); fflush(log); }
#endif
		RwTexDictionaryAddTexture(texDict, tex);
	}

#ifdef WEBOS_TOUCHPAD
	if (log) { fprintf(log, "RwTexDictionaryGtaStreamRead: All textures loaded successfully\n"); fflush(log); fclose(log); }
#endif
	return texDict;
}
