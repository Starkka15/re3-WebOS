...
void CTheScripts::Process()
{
	...
	if (log) { fprintf(log, "CTheScripts::Process: START\n"); fflush(log); fclose(log); }
#endif
	...
	if (log) { fprintf(log, "CTheScripts::Process: About to UpsideDownCars.UpdateTimers()\n"); fflush(log); fclose(log); }
#endif
	UpsideDownCars.UpdateTimers();
#ifdef WEBOS_VERBOSE_DEBUG_DISABLED
	log = NULL /* logging disabled */;
	if (log) { fprintf(log, "CTheScripts::Process: About to StuckCars.Process()\n"); fflush(log); fclose(log); }
#endif
	StuckCars.Process();
#ifdef WEBOS_VERBOSE_DEBUG_DISABLED
	log = NULL /* logging disabled */;
	if (log) { fprintf(log, "CTheScripts::Process: About to DrawScriptSpheres()\n"); fflush(log); fclose(log); }
#endif
	DrawScriptSpheres();
#ifdef WEBOS_VERBOSE_DEBUG_DISABLED
	log = NULL /* logging disabled */;
	if (log) { fprintf(log, "CTheScripts::Process: DrawScriptSpheres() completed\n"); fflush(log); fclose(log); }
#endif
...
#ifdef WEBOS_VERBOSE_DEBUG_DISABLED
	log = NULL /* logging disabled */;
	if (log) { fprintf(log, "CTheScripts::Process: About to process scripts, pActiveScripts=%p\n", pActiveScripts); fflush(log); fclose(log); }
#endif
...
#ifdef WEBOS_VERBOSE_DEBUG_DISABLED
		log = NULL /* logging disabled */;
		if (log) { fprintf(log, "CTheScripts::Process: Processing script %p\n", script); fflush(log); fclose(log); }
#endif
		++ScriptsUpdated;
		script->UpdateTimers(timeStep);
#ifdef WEBOS_VERBOSE_DEBUG_DISABLED
		log = NULL /* logging disabled */;
		if (log) { fprintf(log, "CTheScripts::Process: About to call script->Process() for script %p\n", script); fflush(log); fclose(log); }
#endif
		script->Process();
#ifdef WEBOS_VERBOSE_DEBUG_DISABLED
		log = NULL /* logging disabled */;
		if (log) { fprintf(log, "CTheScripts::Process: script->Process() completed for script %p\n", script); fflush(log); fclose(log); }
#endif
		script = next;
	}
	DbgFlag = false;
...
#ifdef WEBOS_VERBOSE_DEBUG_DISABLED
	log = NULL /* logging disabled */;
	if (log) { fprintf(log, "CTheScripts::Process: All scripts processed, RETURNING\n"); fflush(log); fclose(log); }
#endif
...
#ifdef WEBOS_TOUCHPAD
		// WEBOS: Disabled excessive script loop logging to prevent FD exhaustion
