...
int8 CRunningScript::ProcessCommands300To399(int32 command)
{
	...
	*/
	...
	{
		CollectParameters(&m_nIp, 2);
		// Ensure fade time is at least 10ms to avoid instant completion that stops ProcessFade
		float fadeTime = ScriptParams[0] / 1000.0f;
		if(fadeTime < 0.01f) fadeTime = 0.01f;
		TheCamera.Fade(fadeTime, ScriptParams[1]);
		return 0;
	}
