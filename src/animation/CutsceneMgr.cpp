...
#ifdef WEBOS_TOUCHPAD_DISABLED_AUTO_SKIP
	// WebOS: Auto-skip DISABLED - let in-game cutscenes play normally
	// User can skip by touching screen (see Update() function)
	// Keeping this code for reference in case of future issues
...
CCutsceneMgr::DeleteCutsceneData(void)
{
	...
	for (--ms_numCutsceneObjs; ms_numCutsceneObjs >= 0; ms_numCutsceneObjs--) {
		// WEBOS: Removed per-object logging to avoid file descriptor exhaustion
	...
	if (log) {
		fprintf(log, "DeleteCutsceneData: COMPLETE\n");
		fclose(log);
	}
#endif
}
...
CCutsceneMgr::Update(void)
{
	...
	if(lastLoadStatus != ms_cutsceneLoadStatus) {
		...
		lastLoadStatus = ms_cutsceneLoadStatus;
	}
	updateCount++;
#endif
...
#ifdef WEBOS_TOUCHPAD
			// WebOS: Add touch screen support for skipping in-game cutscenes
			|| CPad::GetPad(0)->GetLeftMouseJustDown()  // Touch is mapped to mouse on webOS
#endif
			)
			FinishCutscene();
	}
