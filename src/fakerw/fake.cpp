...
RwRaster *RwRasterSetFromImage(RwRaster *raster, RwImage *image) {
	return raster->setFromImage(image);
}
...
RwBool rwNativeTextureHackRead(RwStream *stream, RwTexture **tex, RwInt32 size)
{
	...
	*tex = Texture::streamReadNative(stream);
	...
	if (log) { fprintf(log, "rwNativeTextureHackRead: streamReadNative returned tex=%p\n", *tex); fflush(log); fclose(log); }
#endif

	// Check if texture was successfully read before accessing it
	if (*tex == nil) {
#ifdef WEBOS_TOUCHPAD
		log = NULL /* logging disabled */;
		if (log) { fprintf(log, "rwNativeTextureHackRead: streamReadNative failed, returning false\n"); fflush(log); fclose(log); }
#endif
		return false;
	}
...
#ifdef WEBOS_TOUCHPAD
	log = NULL /* logging disabled */;
	if (log) { fprintf(log, "rwNativeTextureHackRead: About to convertTexToCurrentPlatform, raster=%p\n", (*tex)->raster); fflush(log); fclose(log); }
#endif
	(*tex)->raster = rw::Raster::convertTexToCurrentPlatform((*tex)->raster);
#ifdef WEBOS_TOUCHPAD
	log = NULL /* logging disabled */;
	if (log) { fprintf(log, "rwNativeTextureHackRead: convertTexToCurrentPlatform returned raster=%p\n", (*tex)->raster); fflush(log); fclose(log); }
#endif
#endif

#ifdef WEBOS_TOUCHPAD
	log = NULL /* logging disabled */;
	if (log) { fprintf(log, "rwNativeTextureHackRead: COMPLETED, returning %d\n", *tex != nil); fflush(log); fclose(log); }
#endif
