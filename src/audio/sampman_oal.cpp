...
#include "../core/config.h"  // Must be included first for AUDIO_OAL_NO_EFX
...
#include <shlguid.h>
#else
// Non-Windows (Linux, WebOS)
#include <AL/al.h>
#include <AL/alc.h>
#ifndef AUDIO_OAL_NO_EFX
#include <AL/alext.h>
#include <AL/efx.h>
#include <AL/efx-presets.h>
#endif
...
add_providers()
{
	...
	strcpy(providers[0].name, "WebOS OpenAL");
	providers[0].sources = 32; // Reasonable default for mobile
	providers[0].bSupportsFx = false; // No EFX on webOS
	...
	if (log) {
		fprintf(log, "[add_providers] WebOS provider added: name=%s, sources=%d\n",
				providers[0].name, providers[0].sources);
		fflush(log); fclose(log);
	}
#else
...
cSampleManager::Initialise(void)
{
	...
	FILE *log;
	log = NULL /* logging disabled */;
	if (log) { fprintf(log, "[SampleManager::Initialise] Starting audio initialization\n"); fflush(log); fclose(log); }
#endif
...
#ifdef WEBOS_TOUCHPAD
	log = NULL /* logging disabled */;
	if (log) { fprintf(log, "[SampleManager::Initialise] Skipping EFXInit (AUDIO_OAL_NO_EFX)\n"); fflush(log); fclose(log); }
#endif
#endif

#ifdef WEBOS_TOUCHPAD
	log = NULL /* logging disabled */;
	if (log) { fprintf(log, "[SampleManager::Initialise] Creating CStream objects...\n"); fflush(log); fclose(log); }
#endif
...
#ifdef WEBOS_TOUCHPAD
	log = NULL /* logging disabled */;
	if (log) { fprintf(log, "[SampleManager::Initialise] Calling CStream::Initialise...\n"); fflush(log); fclose(log); }
#endif
...
#ifdef WEBOS_TOUCHPAD
	log = NULL /* logging disabled */;
	if (log) { fprintf(log, "[SampleManager::Initialise] Calling add_providers...\n"); fflush(log); fclose(log); }
#endif
...
#ifdef WEBOS_TOUCHPAD
	log = NULL /* logging disabled */;
	if (log) { fprintf(log, "[SampleManager::Initialise] add_providers done\n"); fflush(log); fclose(log); }
#endif

	{
		...
		log = NULL /* logging disabled */;
		if (log) {
			fprintf(log, "[SampleManager::Initialise] Provider: %s, max sources: %d\n",
					providers[index].id ? providers[index].id : "NULL", _maxSamples);
			fflush(log); fclose(log);
		}
#endif
...
#ifdef WEBOS_TOUCHPAD
		log = NULL /* logging disabled */;
		if (log) { fprintf(log, "[SampleManager::Initialise] Calling alcOpenDevice...\n"); fflush(log); fclose(log); }
#endif
...
#ifdef WEBOS_TOUCHPAD
		log = NULL /* logging disabled */;
		if (log) { fprintf(log, "[SampleManager::Initialise] alcOpenDevice returned: %p\n", (void*)ALDevice); fflush(log); fclose(log); }
#endif

#ifdef WEBOS_TOUCHPAD
		log = NULL /* logging disabled */;
		if (log) { fprintf(log, "[SampleManager::Initialise] Calling alcCreateContext...\n"); fflush(log); fclose(log); }
#endif
...
#ifdef WEBOS_TOUCHPAD
		log = NULL /* logging disabled */;
		if (log) { fprintf(log, "[SampleManager::Initialise] alcCreateContext returned: %p\n", (void*)ALContext); fflush(log); fclose(log); }
#endif

#ifdef WEBOS_TOUCHPAD
		log = NULL /* logging disabled */;
		if (log) { fprintf(log, "[SampleManager::Initialise] Calling alcMakeContextCurrent...\n"); fflush(log); fclose(log); }
#endif
...
#ifndef AUDIO_OAL_NO_EFX
		// Only require AL_SOFT_loop_points when using EFX (effects)
		...
		if (log) {
			...
			if (strstr(ext,"AL_SOFT_loop_points") == NULL) {
				fprintf(log, "[SampleManager] WARNING: AL_SOFT_loop_points not available (OK for webOS)\n");
			}
			fflush(log); fclose(log);
		}
#endif
...
#ifdef WEBOS_TOUCHPAD
	log = NULL /* logging disabled */;
	if (log) { fprintf(log, "[SampleManager::Initialise] Audio initialization COMPLETE!\n"); fflush(log); fclose(log); }
#endif
