...
CClouds::Render(void)
{
	...
	// Clear any accumulated GL errors from before this function
	while(glGetError() != GL_NO_ERROR);
	FILE* startLog = NULL /* logging disabled */;
	if(startLog) { fprintf(startLog, "CClouds::Render START - GL errors cleared\n"); fclose(startLog); }
#endif
...
#ifdef WEBOS_VERBOSE_DEBUG_DISABLED
	{ GLenum e = glGetError(); if(e) { FILE* f = NULL /* logging disabled */; if(f) { fprintf(f, "GL ERROR after ZTESTENABLE: 0x%x\n", e); fclose(f); } } }
#endif
