...
#ifdef WEBOS_VERBOSE_DEBUG_DISABLED
	if (log) { fprintf(log, "AddTxdSlot returned slot %d\n", slot); fflush(log); }
#endif
...
#else
#ifdef WEBOS_VERBOSE_DEBUG_DISABLED
	if (log) { fprintf(log, "About to call LoadTxd for FONTS.TXD...\n"); fflush(log); }
#endif
	CTxdStore::LoadTxd(slot, "MODELS/FONTS.TXD");
#ifdef WEBOS_VERBOSE_DEBUG_DISABLED
	if (log) { fprintf(log, "LoadTxd completed\n"); fflush(log); }
#endif
#endif

#ifdef WEBOS_VERBOSE_DEBUG_DISABLED
	if (log) { fprintf(log, "About to call AddRef...\n"); fflush(log); }
#endif
	CTxdStore::AddRef(slot);
#ifdef WEBOS_VERBOSE_DEBUG_DISABLED
	if (log) { fprintf(log, "About to call PushCurrentTxd...\n"); fflush(log); }
#endif
	CTxdStore::PushCurrentTxd();
#ifdef WEBOS_VERBOSE_DEBUG_DISABLED
	if (log) { fprintf(log, "About to call SetCurrentTxd...\n"); fflush(log); }
#endif
	CTxdStore::SetCurrentTxd(slot);
#ifdef WEBOS_VERBOSE_DEBUG_DISABLED
	if (log) { fprintf(log, "About to set Sprite[0] texture...\n"); fflush(log); }
#endif
	Sprite[0].SetTexture("font2", "font2_mask");
#ifdef WEBOS_VERBOSE_DEBUG_DISABLED
	if (log) { fprintf(log, "Sprite[0] texture set, about to set Sprite[1]...\n"); fflush(log); }
#endif
#ifdef MORE_LANGUAGES
	if (IsJapanese()) {
...
#ifdef WEBOS_VERBOSE_DEBUG_DISABLED
	if (log) { fprintf(log, "Sprite[1] texture set, about to set Sprite[2]...\n"); fflush(log); }
#endif
	Sprite[2].SetTexture("font1", "font1_mask");
#ifdef WEBOS_VERBOSE_DEBUG_DISABLED
	if (log) { fprintf(log, "All sprite textures set, calling SetScale...\n"); fflush(log); }
#endif
	SetScale(1.0f, 1.0f);
...
#ifdef WEBOS_VERBOSE_DEBUG_DISABLED
	if (log) { fprintf(log, "About to call LoadButtons...\n"); fflush(log); }
#endif
	LoadButtons("MODELS/X360BTNS.TXD");
#ifdef WEBOS_VERBOSE_DEBUG_DISABLED
	if (log) { fprintf(log, "LoadButtons completed\n"); fflush(log); }
#endif
#endif

#ifdef WEBOS_VERBOSE_DEBUG_DISABLED
	if (log) { fprintf(log, "CFont::Initialise completed successfully\n"); fflush(log); fclose(log); }
#endif
}
...
CFont::InitPerFrame(void)
{
	...
	if (log) { fprintf(log, "CFont::InitPerFrame: GetBank for Sprite[0] succeeded, calling for Sprite[1]\n"); fflush(log); fclose(log); }

	CSprite2d::GetBank(15, Sprite[1].m_pTexture);

	log = NULL /* logging disabled */;
	if (log) { fprintf(log, "CFont::InitPerFrame: GetBank for Sprite[1] succeeded, calling for Sprite[2]\n"); fflush(log); fclose(log); }

	CSprite2d::GetBank(15, Sprite[2].m_pTexture);

	log = NULL /* logging disabled */;
	if (log) { fprintf(log, "CFont::InitPerFrame: All GetBank calls succeeded\n"); fflush(log); fclose(log); }

#ifdef MORE_LANGUAGES
	log = NULL /* logging disabled */;
	if (log) { fprintf(log, "CFont::InitPerFrame: About to check IsJapanese\n"); fflush(log); fclose(log); }
	if (IsJapanese())
		CSprite2d::GetBank(15, Sprite[3].m_pTexture);
	log = NULL /* logging disabled */;
	if (log) { fprintf(log, "CFont::InitPerFrame: Japanese check done\n"); fflush(log); fclose(log); }
#endif

	log = NULL /* logging disabled */;
	if (log) { fprintf(log, "CFont::InitPerFrame: About to call SetDropShadowPosition(0)\n"); fflush(log); fclose(log); }
	SetDropShadowPosition(0);

	log = NULL /* logging disabled */;
	if (log) { fprintf(log, "CFont::InitPerFrame: About to set NewLine = false\n"); fflush(log); fclose(log); }
	NewLine = false;

#ifdef BUTTON_ICONS
	log = NULL /* logging disabled */;
	if (log) { fprintf(log, "CFont::InitPerFrame: About to set PS2Symbol\n"); fflush(log); fclose(log); }
	PS2Symbol = BUTTON_NONE;
	log = NULL /* logging disabled */;
	if (log) { fprintf(log, "CFont::InitPerFrame: PS2Symbol set\n"); fflush(log); fclose(log); }
#endif

	log = NULL /* logging disabled */;
	if (log) { fprintf(log, "CFont::InitPerFrame: About to return from function\n"); fflush(log); fclose(log); }
}
