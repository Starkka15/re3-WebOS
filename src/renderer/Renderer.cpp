...
CRenderer::RenderOneNonRoad(CEntity *e)
{
	...
	// CRITICAL: Skip entities that don't have geometry loaded yet
	// (Entities are created before their m_rwObject is loaded from disk)
	if (e == NULL || e->m_rwObject == NULL) {
		...
		return; // Skip this entity for now, will render when geometry loads
	}
#endif
...
#ifdef WEBOS_TOUCHPAD
	// Skip assert - allow rendering to continue if type doesn't match
	if(RwObjectGetType(ent->m_rwObject) != rpATOMIC)
		return;
#else
...
CRenderer::SetupEntityVisibility(CEntity *ent)
{
	...
	if(a){
		...
		if(ent->m_rwObject == nil){
#ifdef WEBOS_TOUCHPAD
			// webOS: Only load geometry for nearby entities to prevent memory exhaustion
			// Don't load geometry for entities beyond streaming distance
			if(dist > STREAM_DISTANCE){
				return VIS_INVISIBLE; // Too far, don't load yet
			}
#endif
			ent->CreateRwObject();
		}
#ifdef WEBOS_TOUCHPAD
		// Skip if CreateRwObject() failed instead of crashing
		if(ent->m_rwObject == nil)
			return VIS_INVISIBLE;
#else
	...
	if (renderListCount < 100) {
		...
		if (log) {
			...
			if (renderListCount < 10) {
				for (int i = 0; i < Min(5, ms_nNoOfVisibleEntities); i++) {
					CEntity* ent = ms_aVisibleEntityPtrs[i];
					if (ent != NULL) {
						fprintf(log, "  Entity[%d]: rwObject=%p IsPed=%d IsVehicle=%d\n",
							i, ent->m_rwObject, ent->IsPed(), ent->IsVehicle());
					} else {
						fprintf(log, "  Entity[%d]: NULL!\n", i);
					}
				}
			}
			fflush(log); fclose(log);
