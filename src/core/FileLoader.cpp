...
CFileLoader::LoadLevel(const char *filename)
{
	...
	for(line = LoadLine(fd); line; line = LoadLine(fd)){
		...
		if(strncmp(line, "IMAGEPATH", 9) == 0){
			...
			if (log) { fprintf(log, "CFileLoader::LoadLevel: About to load IDE file '%s'\n", line + 4); fflush(log); fclose(log); }
#endif
			LoadingScreenLoadingFile(line + 4);
#ifdef WEBOS_TOUCHPAD
			log = NULL /* logging disabled */;
			if (log) { fprintf(log, "CFileLoader::LoadLevel: About to call LoadObjectTypes\n"); fflush(log); fclose(log); }
#endif
			LoadObjectTypes(line + 4);
#ifdef WEBOS_TOUCHPAD
			log = NULL /* logging disabled */;
			if (log) { fprintf(log, "CFileLoader::LoadLevel: LoadObjectTypes completed for '%s'\n", line + 4); fflush(log); fclose(log); }
#endif
...
#ifdef WEBOS_TOUCHPAD
	// WEBOS: Check if file opened successfully (can fail due to FD exhaustion)
	if(fd == 0) {
		printf("LoadCollisionFile: Failed to open '%s' (likely out of file descriptors)\n", filename);
		POP_MEMID();
		return;
	}
#endif
...
CFileLoader::LoadObjectTypes(const char *filename)
{
...
#ifdef WEBOS_TOUCHPAD
	log = NULL /* logging disabled */;
	if (log) { fprintf(log, "LoadObjectTypes: File opened, fd=%d, starting parse loop\n", fd); fflush(log); fclose(log); }
#endif
	for(line = CFileLoader::LoadLine(fd); line; line = CFileLoader::LoadLine(fd)){
		...
		if(section == NONE){
			if(isLine4(line, 'o','b','j','s')) {
				section = OBJS;
#ifdef WEBOS_TOUCHPAD
				log = NULL /* logging disabled */;
				if (log) { fprintf(log, "LoadObjectTypes: Entering OBJS section\n"); fflush(log); fclose(log); }
#endif
			}
			else if(isLine4(line, 't','o','b','j')) {
				section = TOBJ;
#ifdef WEBOS_TOUCHPAD
				log = NULL /* logging disabled */;
				if (log) { fprintf(log, "LoadObjectTypes: Entering TOBJ section\n"); fflush(log); fclose(log); }
#endif
			}
			else if(isLine4(line, 'h','i','e','r')) {
				section = HIER;
#ifdef WEBOS_TOUCHPAD
				log = NULL /* logging disabled */;
				if (log) { fprintf(log, "LoadObjectTypes: Entering HIER section\n"); fflush(log); fclose(log); }
#endif
			}
			else if(isLine4(line, 'c','a','r','s')) {
				section = CARS;
#ifdef WEBOS_TOUCHPAD
				log = NULL /* logging disabled */;
				if (log) { fprintf(log, "LoadObjectTypes: Entering CARS section\n"); fflush(log); fclose(log); }
#endif
			}
			else if(isLine4(line, 'p','e','d','s')) {
				section = PEDS;
#ifdef WEBOS_TOUCHPAD
				log = NULL /* logging disabled */;
				if (log) { fprintf(log, "LoadObjectTypes: Entering PEDS section\n"); fflush(log); fclose(log); }
#endif
			}
			else if(isLine4(line, 'p','a','t','h')) {
				section = PATH;
#ifdef WEBOS_TOUCHPAD
				log = NULL /* logging disabled */;
				if (log) { fprintf(log, "LoadObjectTypes: Entering PATH section\n"); fflush(log); fclose(log); }
#endif
			}
			else if(isLine4(line, '2','d','f','x')) {
				section = TWODFX;
#ifdef WEBOS_TOUCHPAD
				log = NULL /* logging disabled */;
				if (log) { fprintf(log, "LoadObjectTypes: Entering TWODFX section\n"); fflush(log); fclose(log); }
#endif
			}
...
#ifdef WEBOS_TOUCHPAD
	log = NULL /* logging disabled */;
	if (log) { fprintf(log, "LoadPedObject: Parsed id=%d, model=%s, about to AddPedModel\n", id, model); fflush(log); fclose(log); }
#endif
	mi = CModelInfo::AddPedModel(id);
#ifdef WEBOS_TOUCHPAD
	log = NULL /* logging disabled */;
	if (log) { fprintf(log, "LoadPedObject: AddPedModel returned, setting model name\n"); fflush(log); fclose(log); }
#endif
	mi->SetModelName(model);
	mi->SetTexDictionary(txd);
...
#ifdef WEBOS_TOUCHPAD
	log = NULL /* logging disabled */;
	if (log) { fprintf(log, "LoadPedObject: About to FindPedType\n"); fflush(log); fclose(log); }
#endif
	mi->m_pedType = CPedType::FindPedType(pedType);
#ifdef WEBOS_TOUCHPAD
	log = NULL /* logging disabled */;
	if (log) { fprintf(log, "LoadPedObject: About to GetPedStatType\n"); fflush(log); fclose(log); }
#endif
...
#ifdef WEBOS_TOUCHPAD
	log = NULL /* logging disabled */;
	if (log) { fprintf(log, "LoadScene: File opened, fd=%d, starting parse\n", fd); fflush(log); fclose(log); }
#endif
	for(line = CFileLoader::LoadLine(fd); line; line = CFileLoader::LoadLine(fd)){
		if(*line == '\0' || *line == '#')
			continue;
		...
		if(section == NONE){
			if(isLine4(line, 'i','n','s','t')) {
				section = INST;
#ifdef WEBOS_TOUCHPAD
				log = NULL /* logging disabled */;
				if (log) { fprintf(log, "LoadScene: Entering INST section\n"); fflush(log); fclose(log); }
#endif
			}
			else if(isLine4(line, 'z','o','n','e')) {
				section = ZONE;
#ifdef WEBOS_TOUCHPAD
				log = NULL /* logging disabled */;
				if (log) { fprintf(log, "LoadScene: Entering ZONE section\n"); fflush(log); fclose(log); }
#endif
			}
			else if(isLine4(line, 'c','u','l','l')) {
				section = CULL;
#ifdef WEBOS_TOUCHPAD
				log = NULL /* logging disabled */;
				if (log) { fprintf(log, "LoadScene: Entering CULL section\n"); fflush(log); fclose(log); }
#endif
			}
			else if(isLine4(line, 'p','i','c','k')) {
				section = PICK;
#ifdef WEBOS_TOUCHPAD
				log = NULL /* logging disabled */;
				if (log) { fprintf(log, "LoadScene: Entering PICK section\n"); fflush(log); fclose(log); }
#endif
			}
			else if(isLine4(line, 'p','a','t','h')) {
				section = PATH;
#ifdef WEBOS_TOUCHPAD
				log = NULL /* logging disabled */;
				if (log) { fprintf(log, "LoadScene: Entering PATH section\n"); fflush(log); fclose(log); }
#endif
			}
		}else if(isLine3(line, 'e','n','d')){
			section = NONE;
...
CFileLoader::LoadObjectInstance(const char *line)
{
	...
	if(instanceCount <= 20) {
		...
		if (log) {
			...
			fprintf(log, "  ModelInfo=%p IsBigBuilding=%d\n", mi, mi->m_isBigBuilding);
			fflush(log); fclose(log);
		}
	}
#endif
	...
	if(mi->GetObjectID() == -1){
		...
		if((instanceCount % 100) == 0) {
			...
			if (log) {
				...
				int treadableUsed = CPools::GetTreadablePool()->GetNoOfUsedSpaces();
				int treadableSize = CPools::GetTreadablePool()->GetSize();
				fprintf(log, "LoadObjectInstance: Pool usage - Buildings: %d/%d, Treadables: %d/%d\n",
					buildingUsed, buildingSize, treadableUsed, treadableSize);
				fflush(log);
				fclose(log);
			}
		}
