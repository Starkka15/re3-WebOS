...
// Track load failures to prevent infinite retry loops
static uint8 ms_loadFailureCount[NUMSTREAMINFO] = {0};
#define MAX_LOAD_FAILURES 3
#endif
...
#define MB (1024*1024)
	...
	// webOS TouchPad has 1GB RAM - allocate 256MB for streaming
	ms_memoryAvailable = 256*MB;
...
#endif
	// FIX: Process images in forward order (0, 1, 2...) so later images can override earlier ones
	// The original code: while(i-- >= 1) processed in REVERSE order, causing txd.dir to override gta3.dir
	for(int j = 0; j < i; j++){
#ifdef WEBOS_VERBOSE_DEBUG_DISABLED
		{
			FILE *log = NULL; // fopen("/media/internal/.gta3/debug.log", "a");
...
#endif
		strcpy(dirname, CdStreamGetImageName(j));
...
CStreaming::ConvertBufferToObject(int8 *buf, int32 streamId)
{
		...
		if(!success){
			...
			if (log) {
				fprintf(log, "Streaming: Failed to load %s.txd (attempt %d/%d)\n",
					...
					ms_loadFailureCount[streamId], MAX_LOAD_FAILURES);
				fflush(log);
				fclose(log);
			}
			if (ms_loadFailureCount[streamId] < MAX_LOAD_FAILURES) {
				RemoveModel(streamId);
				ReRequestModel(streamId);
			} else {
				log = NULL /* logging disabled */;
				if (log) {
					fprintf(log, "Streaming: Giving up on %s.txd after %d failures, skipping\n",
						CTxdStore::GetTxdName(streamId - STREAM_OFFSET_TXD),
						MAX_LOAD_FAILURES);
					fflush(log);
					fclose(log);
				...
				RemoveModel(streamId);
				// Don't retry - just skip this texture
			}
#else
			...
			ms_loadFailureCount[streamId] = 0;
		}
#endif
	}
	...
	if(!success){
#ifdef WEBOS_VERBOSE_DEBUG_DISABLED
		// This is a fallback error path - use the same failure tracking
		if (ms_loadFailureCount[streamId] < MAX_LOAD_FAILURES) {
			ReRequestModel(streamId);
		}
		// else: Already logged and handled above, just skip retry
#else
		ReRequestModel(streamId);
#endif
		if(streamId < STREAM_OFFSET_TXD)
		...
		if(i < 10) {
			...
			if (log) {
				fprintf(log, "  Building[%d]: level=%d, isBig=%d, pos=(%.1f,%.1f,%.1f)\n",
					...
					b ? b->GetPosition().x : 0, b ? b->GetPosition().y : 0, b ? b->GetPosition().z : 0);
				fflush(log); fclose(log);
			}
		}
#endif
...
#ifdef WEBOS_TOUCHPAD
		    && (loadAllLevels ? (requestCount < MAX_BUILDINGS_NEUTRAL) : (b->m_level == level))
			...
			if(loadCallCount == 3) {
				...
				if (log) {
					const char* name = streamId < STREAM_OFFSET_TXD ?
						CModelInfo::GetModelInfo(streamId)->GetModelName() :
						CTxdStore::GetTxdName(streamId - STREAM_OFFSET_TXD);
					fprintf(log, "LoadAllRequestedModels #3: Loading %s (ID=%d, size=%d sectors)\n",
						name, streamId, size);
					fflush(log); fclose(log);
				}
			}
	...
	if(makeSpaceCallCount <= 20 || (makeSpaceCallCount % 100) == 0) {
		...
		if (log) {
			fprintf(log, "MakeSpaceFor #%d: Need %d bytes, Used=%d MB, Available=%d MB, Free=%d MB\n",
				...
				(ms_memoryAvailable - ms_memoryUsed)/(1024*1024));
			fflush(log); fclose(log);
		}
	}
#endif
