...
CCamera::ProcessFade(void)
{
	...
	static uint32 lastLoggedTime = 0;
	uint32 currentTime = CTimer::GetTimeInMilliseconds();
	// Log when time changes OR first 20 entries
	if((fadeLogCount < 20 || currentTime != lastLoggedTime) && fadeLogCount < 200) {
		FILE* log = fopen("/media/internal/.gta3/debug.log", "a");
		if(log) {
			float fade = (currentTime - m_uiFadeTimeStarted)/1000.0f;
			fprintf(log, "ProcessFade: m_bFading=%d, FadeValue=%d, Direction=%d, TimeToFadeOut=%.2f, fade=%.3f, TimeStarted=%u, CurrentTime=%u\n",
				m_bFading ? 1 : 0, CDraw::FadeValue, m_iFadingDirection, m_fTimeToFadeOut, fade,
				m_uiFadeTimeStarted, currentTime);
			fflush(log);
			fclose(log);
			fadeLogCount++;
...
CCamera::Fade(float timeout, int16 direction)
{
	...
	static int fadeCallCount = 0;
	if(fadeCallCount++ < 50) {
		FILE* log = NULL /* logging disabled */;
		if(log) {
			fprintf(log, "CCamera::Fade() CALLED: timeout=%.2f, direction=%d (%s), Time=%u\n",
				timeout, direction, direction == FADE_IN ? "FADE_IN" : "FADE_OUT",
