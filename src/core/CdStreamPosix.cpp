...
#if defined(__linux__) && !defined(WEBOS_TOUCHPAD)
...
void *CdStreamThread(void *param)
{
	...
	while (gpReadInfo[channel].nThreadStatus != 2){
		...
		if ( pChannel->nStatus == STREAM_NONE )
		{
			...
			lseek(pChannel->hFile, seekOffset, SEEK_SET);
#else
			lseek(pChannel->hFile, (size_t)pChannel->nSectorOffset * (size_t)CDSTREAM_SECTOR_SIZE, SEEK_SET);
#endif
			ssize_t bytesRead = read(pChannel->hFile, pChannel->pBuffer, pChannel->nSectorsToRead * CDSTREAM_SECTOR_SIZE);
			if (bytesRead == -1) {
				...
				if (log) {
					fprintf(log, "CdStream: Read %zd bytes, first 16 bytes: ", bytesRead);
					for (int i = 0; i < 16 && i < bytesRead; i++) {
						fprintf(log, "%02X ", ((uint8_t*)pChannel->pBuffer)[i]);
					}
					fprintf(log, "\n");
					fflush(log);
					fclose(log);
...
CdStreamAddImage(char const *path)
{
#ifdef WEBOS_TOUCHPAD
	{
		...
		if (log) {
			...
			fprintf(log, "CdStreamAddImage: START, path='%s', gNumImages=%d\n", path, gNumImages);
			fprintf(log, "CdStreamAddImage: Current directory='%s'\n", cwd);
			fprintf(log, "CdStreamAddImage: Flags=0x%x\n", _gdwCdStreamFlags);
			fflush(log);
			fclose(log);
		}
	}
...
#ifdef WEBOS_TOUCHPAD
	{
		...
		if (log) {
			...
			// Read first 16 bytes to verify file
			int testFd = gImgFiles[gNumImages] - 1;
			uint8_t testBuf[16];
			lseek(testFd, 0, SEEK_SET);
			ssize_t nr = read(testFd, testBuf, 16);
			fprintf(log, "CdStreamAddImage: First 16 bytes of file: ");
			for (int i = 0; i < 16 && i < nr; i++) {
				fprintf(log, "%02X ", testBuf[i]);
			}
			fprintf(log, "\n");
			fflush(log);
			fclose(log);
