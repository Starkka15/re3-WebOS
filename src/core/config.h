...
#if defined(__SWITCH__) || defined(PSP2) || defined(WEBOS_TOUCHPAD)
	#define GTA_HANDHELD
#endif

// WebOS TouchPad specific optimizations
#ifdef WEBOS_TOUCHPAD
	#define IGNORE_MOUSE_KEYBOARD // Touch-only interface

	// Disable debug menu to avoid font crashes
	#undef DEBUGMENU

	// Disable OpenAL EFX (webOS PDK OpenAL doesn't support EFX extension)
	#define AUDIO_OAL_NO_EFX

	// Performance optimizations for limited RAM (512MB-2GB)
	#define SQUEEZE_PERFORMANCE
	#define VC_RAIN_NERF  // Reduce rain particle effects
	#define PC_PARTICLE   // Use simpler PC particles instead of PS2

	// Disable expensive graphics features - moved to after LIBRW block
	...
	// Debug menu disabled for webOS (causes font crashes)
	// #define DEBUGMENU - DISABLED for WEBOS_TOUCHPAD
	#define CHATTYSPLASH  // Print loading progress
	#define TIMEBARS      // Show FPS/performance
#endif
...
#define COMPRESSED_COL_VECTORS	// currently need compressed vectors in this code
	...
	#ifndef WEBOS_TOUCHPAD  // Don't define DEBUGMENU for webOS (causes font crashes)
	#define DEBUGMENU
	#endif
#endif
...
// WebOS: Disable expensive graphics features (must come after LIBRW block)
#ifdef WEBOS_TOUCHPAD
#undef EXTENDED_PIPELINES
#undef SCREEN_DROPLETS
#undef NEW_RENDERER
...
#		ifndef WEBOS_TOUCHPAD  // webOS: Disable NO_ISLAND_LOADING to reduce memory usage
...
// Peds
#ifndef WEBOS_TOUCHPAD  // Adreno 220 doesn't support dynamic array indexing in GLSL ES 1.00
