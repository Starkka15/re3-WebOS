...
void CPad::UpdatePads(void)
{
	...
	if (log) { fprintf(log, "CPad::UpdatePads: About to call UpdateMouse()\n"); fflush(log); fclose(log); }
#endif
...
#ifdef WEBOS_VERBOSE_DEBUG_DISABLED
	log = NULL /* logging disabled */;
	if (log) { fprintf(log, "CPad::UpdatePads: UpdateMouse() done\n"); fflush(log); fclose(log); }
#endif

#ifdef XINPUT
	GetPad(0)->AffectFromXinput(m_bMapPadOneToPadTwo ? 1 : 0);
	GetPad(1)->AffectFromXinput(m_bMapPadOneToPadTwo ? 0 : 1);
#elif !defined RW_NULL
#ifdef WEBOS_VERBOSE_DEBUG_DISABLED
	log = NULL /* logging disabled */;
	if (log) { fprintf(log, "CPad::UpdatePads: About to call CapturePad(0)\n"); fflush(log); fclose(log); }
#endif
	CapturePad(0);
#ifdef WEBOS_VERBOSE_DEBUG_DISABLED
	log = NULL /* logging disabled */;
	if (log) { fprintf(log, "CPad::UpdatePads: CapturePad(0) done\n"); fflush(log); fclose(log); }
#endif
#endif
...
#ifdef WEBOS_VERBOSE_DEBUG_DISABLED
	log = NULL /* logging disabled */;
	if (log) { fprintf(log, "CPad::UpdatePads: Before ControlsManager calls\n"); fflush(log); fclose(log); }
#endif
...
#ifdef WEBOS_VERBOSE_DEBUG_DISABLED
		log = NULL /* logging disabled */;
		if (log) { fprintf(log, "CPad::UpdatePads: After ClearSimButtonPressCheckers\n"); fflush(log); fclose(log); }
#endif
		ControlsManager.AffectPadFromKeyBoard();
#ifdef WEBOS_VERBOSE_DEBUG_DISABLED
		log = NULL /* logging disabled */;
		if (log) { fprintf(log, "CPad::UpdatePads: After AffectPadFromKeyBoard\n"); fflush(log); fclose(log); }
#endif
		ControlsManager.AffectPadFromMouse();
#ifdef WEBOS_VERBOSE_DEBUG_DISABLED
		log = NULL /* logging disabled */;
		if (log) { fprintf(log, "CPad::UpdatePads: After AffectPadFromMouse\n"); fflush(log); fclose(log); }
#endif
...
#ifdef WEBOS_TOUCHPAD
	// CRITICAL: ProcessVirtualControls must run AFTER Update() to prevent ReconcileTwoControllersInput from overwriting
	...
	log = NULL /* logging disabled */;
	if (log) {
		fprintf(log, "CPad::UpdatePads: COMPLETED. Cross=%d->%d, Square=%d->%d, Triangle=%d->%d\n",
			GetPad(0)->OldState.Cross, GetPad(0)->NewState.Cross,
			GetPad(0)->OldState.Square, GetPad(0)->NewState.Square,
			GetPad(0)->OldState.Triangle, GetPad(0)->NewState.Triangle);

		// Check for JustDown events with game state context
		if (GetPad(0)->GetCrossJustDown()) {
			fprintf(log, "  -> GetCrossJustDown() = TRUE (FIRE/ACTION BUTTON)\n");
			fprintf(log, "     Pad State: DisabledControls=0x%x\n", GetPad(0)->DisablePlayerControls);
		}
		if (GetPad(0)->GetSquareJustDown()) {
			fprintf(log, "  -> GetSquareJustDown() = TRUE (JUMP BUTTON)\n");
			fprintf(log, "     Pad State: DisabledControls=0x%x\n", GetPad(0)->DisablePlayerControls);
		}
		if (GetPad(0)->GetTriangleJustDown()) {
			fprintf(log, "  -> GetTriangleJustDown() = TRUE (ENTER/EXIT BUTTON)\n");
			fprintf(log, "     Pad State: DisabledControls=0x%x\n", GetPad(0)->DisablePlayerControls);
		}
		if (GetPad(0)->GetStartJustDown()) {
			fprintf(log, "  -> GetStartJustDown() = TRUE (PAUSE/MENU BUTTON)\n");
			fprintf(log, "     Pad State: DisabledControls=0x%x\n", GetPad(0)->DisablePlayerControls);
		}
		if (GetPad(0)->GetEnterJustDown())
			fprintf(log, "  -> GetEnterJustDown() = TRUE\n");
