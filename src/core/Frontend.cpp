...
#elif defined RW_GL3 && !defined(WEBOS_TOUCHPAD)
		glfwSetCursorPos(PSGLOBAL(window), SCREEN_WIDTH / 2, SCREEN_HEIGHT / 2);
#endif

#if !defined RW_NULL && !defined(WEBOS_TOUCHPAD)
	// WebOS doesn't have lastMousePos in its platform structure (uses touch instead)
...
CMenuManager::DrawFrontEnd()
{
	...
	if (log) { fprintf(log, "DrawFrontEnd: START, m_nCurrScreen=%d, m_bGameNotLoaded=%d\n", m_nCurrScreen, m_bGameNotLoaded); fflush(log); fclose(log); }
#endif
	...
	if (log) { fprintf(log, "DrawFrontEnd: After screen setup, m_nCurrScreen=%d, about to call DrawFrontEndNormal()\n", m_nCurrScreen); fflush(log); fclose(log); }
#endif
...
#ifdef WEBOS_TOUCHPAD
	log = NULL /* logging disabled */;
	if (log) { fprintf(log, "DrawFrontEnd: DrawFrontEndNormal() completed\n"); fflush(log); fclose(log); }
#endif
...
{
#ifdef WEBOS_TOUCHPAD
	// FIX: Force menu to be fully visible (fade alpha wasn't working)
	m_nMenuFadeAlpha = 255;
	FILE* log = NULL /* logging disabled */;
	if (log) {
		fprintf(log, "DrawFrontEndNormal: START, FORCED m_nMenuFadeAlpha=255\n");
		fflush(log); fclose(log);
	}
#endif
...
#ifdef WEBOS_TOUCHPAD
	log = NULL /* logging disabled */;
	if (log) { fprintf(log, "DrawFrontEndNormal: About to call LoadSplash()\n"); fflush(log); fclose(log); }
#endif
...
#ifdef WEBOS_TOUCHPAD
	log = NULL /* logging disabled */;
	if (log) { fprintf(log, "DrawFrontEndNormal: LoadSplash() completed\n"); fflush(log); fclose(log); }
#endif
...
#ifdef WEBOS_TOUCHPAD
	log = NULL /* logging disabled */;
	if (log) { fprintf(log, "DrawFrontEndNormal: About to set currentSprite, m_nCurrScreen=%d, m_nMenuFadeAlpha=%d\n", m_nCurrScreen, m_nMenuFadeAlpha); fflush(log); fclose(log); }
#endif
...
#ifdef WEBOS_TOUCHPAD
	log = NULL /* logging disabled */;
	if (log) { fprintf(log, "LoadAllTextures: About to call CentreMousePointer()\n"); fflush(log); fclose(log); }
#endif
...
#ifdef WEBOS_TOUCHPAD
	log = NULL /* logging disabled */;
	if (log) { fprintf(log, "LoadAllTextures: About to call DMAudio.ChangeMusicMode()\n"); fflush(log); fclose(log); }
#endif
...
#ifdef WEBOS_TOUCHPAD
	log = NULL /* logging disabled */;
	if (log) { fprintf(log, "LoadAllTextures: About to call DMAudio.PlayFrontEndSound()\n"); fflush(log); fclose(log); }
#endif
...
#ifdef WEBOS_TOUCHPAD
	log = NULL /* logging disabled */;
	if (log) { fprintf(log, "LoadAllTextures: About to set m_nCurrOption\n"); fflush(log); fclose(log); }
#endif
...
#ifdef WEBOS_TOUCHPAD
	log = NULL /* logging disabled */;
	if (log) { fprintf(log, "LoadAllTextures: Initial setup done\n"); fflush(log); fclose(log); }
#endif
...
#ifdef WEBOS_TOUCHPAD
	log = NULL /* logging disabled */;
	if (log) { fprintf(log, "LoadAllTextures: About to load FRONTEND_NSW.TXD, slot=%d\n", frontendTxdSlot); fflush(log); fclose(log); }
	// WebOS TouchPad: Use Nintendo Switch frontend (designed for touchscreen)
	CTxdStore::LoadTxd(frontendTxdSlot, "MODELS/FRONTEND_NSW.TXD");
	log = NULL /* logging disabled */;
	if (log) { fprintf(log, "LoadAllTextures: FRONTEND_NSW.TXD loaded successfully\n"); fflush(log); fclose(log); }
#else
...
#ifdef WEBOS_TOUCHPAD
	log = NULL /* logging disabled */;
	if (log) { fprintf(log, "LoadAllTextures: COMPLETED successfully\n"); fflush(log); fclose(log); }
#endif
}
...
#if defined(RW_GL3) && defined(IMPROVED_VIDEOMODE) && !defined(WEBOS_TOUCHPAD)
